...
int main(){
  ...
  std::vector<GridPoint> point_member;
  point_member.resize(top_num);

  for(GridPoint& p:point_member){
  ...
  for(int i = 0;i < top_num;i++){
    for(int j = i+1;j < top_num;j++){
      for(int k = j + 1;k < top_num;k++){
	im.draw_triangle(point_member[i],point_member[j],point_member[k]);
      }
    }
  }
